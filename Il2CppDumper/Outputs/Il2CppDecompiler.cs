...
namespace Il2CppDumper
{
    public class Il2CppDecompiler
    {
        ...
        public void Decompile(Config config, string outputDir)
        {
            ...
            foreach (var imageDef in metadata.imageDefs)
            {
                ...
                for (int typeDefIndex = imageDef.typeStart; typeDefIndex < typeEnd; typeDefIndex++)
                {
                    ...
                    {
                        var parent = GetTypeByIndex(typeDef.parentIndex);
                        var parentName = parent == null ? $"type_{typeDef.parentIndex}" : executor.GetTypeName(parent, false, false);
                    ...
                    if (typeDef.interfaces_count > 0 && typeDef.interfacesStart >= 0)
                    {
                        ...
                        for (int i = typeDef.interfacesStart; i < interfacesEnd; i++)
                        {
                            ...
                            var @interface = GetTypeByIndex(interfaceIndex);
                            extends.Add(@interface == null ? $"type_{interfaceIndex}" : executor.GetTypeName(@interface, false, false));
                        }
                    }
                    ...
                    if (config.DumpTypeDefIndex)
                        ...
                        writer.Write("\n{");
                    //dump field
                    if (config.DumpField && typeDef.field_count > 0 && typeDef.fieldStart >= 0)
                    {
                        writer.Write("\n\t// Fields\n");
                        ...
                        for (var i = typeDef.fieldStart; i < fieldEnd; ++i)
                        {
                            ...
                            var fieldType = GetTypeByIndex(fieldDef.typeIndex);
                            if (fieldType == null)
                            {
                                writer.Write($"\ttype_{fieldDef.typeIndex} {metadata.GetStringFromIndex(fieldDef.nameIndex)};\n");
                                continue;
                            }
                            var isStatic = false;
                            var isConst = false;
                    ...
                    if (config.DumpProperty && typeDef.property_count > 0)
                    {
                        ...
                        for (var i = typeDef.propertyStart; i < propertyEnd; ++i)
                        {
                            ...
                            if (propertyDef.get >= 0)
                            {
                                ...
                                writer.Write($"{GetMethodReturnTypeName(methodDef)} {metadata.GetStringFromIndex(propertyDef.nameIndex)} {{ ");
                            }
                    ...
                    //dump method
                    if (config.DumpMethod && typeDef.method_count > 0 && typeDef.methodStart >= 0)
                    {
                        writer.Write("\n\t// Methods\n");
                        ...
                        for (var i = typeDef.methodStart; i < methodEnd; ++i)
                        {
                            ...
                            writer.Write($"{GetMethodReturnTypeName(methodDef)} {methodName}(");
                            ...
                            for (var j = 0; j < methodDef.parameterCount; ++j)
                            {
                                ...
                                {
                                    parameterStrs.Add($"var arg{j}");
                                    continue;
                                }
                                var parameterDef = metadata.parameterDefs[parameterIndex];
                                var parameterName = metadata.GetStringFromIndex(parameterDef.nameIndex);
                                var parameterType = GetTypeByIndex(parameterDef.typeIndex);
                                var parameterTypeName = parameterType == null ? $"type_{parameterDef.typeIndex}" : executor.GetTypeName(parameterType, false, false);
                            ...
                            if (il2Cpp.methodDefinitionMethodSpecs.TryGetValue(i, out var methodSpecs))
                            {
                                ...
                                var groups = methodSpecs.Where(x => il2Cpp.methodSpecGenericMethodPointers.ContainsKey(x)).GroupBy(x => il2Cpp.methodSpecGenericMethodPointers[x]);
        ...
        {
            return GetTypeByIndex(methodDef.returnType);
        }

        private string GetMethodReturnTypeName(Il2CppMethodDefinition methodDef)
        {
            var methodReturnType = GetMethodReturnType(methodDef);
            return methodReturnType == null ? "var" : executor.GetTypeName(methodReturnType, false, false);
        }

        private Il2CppType GetTypeByIndex(int index)
        {
            if (index < 0 || index >= il2Cpp.types.Length)
                return null;
            return il2Cpp.types[index];
        }
